 ...
 */
add_filter('wp_get_attachment_image', function ($html, $attachment_id, $size, $icon, array $attr): string|array|null {
    ...
    if ($lazy_loading && ! is_admin()) {
        ...
        $html = preg_replace('/\ssrc="([^"]*)"/', ' data-src="$1" src="'.$svg_placeholder.'"', (string) $html);
    ...
    if (isset($attr['data-width'])) {
        if (preg_match('/width="([^"]*)"/', (string) $html)) {
            $html = preg_replace('/width="([^"]*)"/', 'width="'.esc_attr($attr['data-width']).'"', (string) $html);
        } else {
            $html = preg_replace('/<img/', '<img width="'.esc_attr($attr['data-width']).'"', (string) $html);
        }
    }

    if (isset($attr['data-height'])) {
        if (preg_match('/height="([^"]*)"/', (string) $html)) {
            $html = preg_replace('/height="([^"]*)"/', 'height="'.esc_attr($attr['data-height']).'"', (string) $html);
        } else {
            $html = preg_replace('/<img/', '<img height="'.esc_attr($attr['data-height']).'"', (string) $html);
        }
    }

    // Убираем служебные атрибуты из HTML
    $html = preg_replace('/\sdata-lazy="[^"]*"/', '', (string) $html);
    $html = preg_replace('/\sdata-width="[^"]*"/', '', $html);

    return preg_replace('/\sdata-height="[^"]*"/', '', $html);
}, 10, 5);

add_filter('wp_calculate_image_sizes', function ($sizes, $size, $image_src, array $image_meta, $attachment_id): string {

    $_sizes = '';

    $data_sizes = $image_meta['sizes'] ?? [];
    ...
    return $_sizes . ($image_meta['width'] . 'px');
}, 50, 5);
